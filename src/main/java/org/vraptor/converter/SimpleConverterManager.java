...
public class SimpleConverterManager implements ConverterManager {
	...
	private static final Logger logger = Logger
			.getLogger(SimpleConverterManager.class);

	private LinkedList<Converter> converters = new LinkedList<Converter>();

	private Map<Class, Converter> cachedConverters = new HashMap<Class, Converter>();
	...
	/**
	 * 
	 * @see org.vraptor.converter.ConverterManager#convert(java.lang.String[],
	 *      java.lang.String, java.lang.Class, org.vraptor.scope.LogicRequest,
	 *      java.lang.Class)
	 */
	public Object convert(String[] arrayValue, String completeValue,
			Class<?> originalType, LogicRequest context,
			Class<? extends Converter> overridenConverter)
			throws ConversionException {

		Class<?> type = originalType.isArray() ? originalType
				.getComponentType() : originalType;

		Converter converter = overridenConverter == null ? findConverter(type)
				: getConverter(overridenConverter);
		...
		if (isArray) {
			...
			for (int i = 0; i < arrayValue.length; i++) {
				Array.set(converted, i, converter.convert(arrayValue[i], type,
						context));
			}
			return converted;
	...
	private synchronized Converter getConverter(
			Class<? extends Converter> converter) throws ConversionException {
		try {
			return ReflectionUtil.instantiate(converter);
		} catch (ComponentInstantiationException e) {
			throw new ConversionException("invalid_converter", e.getMessage(),
					e);
		}
	}
	...
	private Converter findConverter(Class<?> type) throws ConversionException {
		...
		if (converter == null) {
			...
			if (converter == null) {
				throw new ConversionException("no_converter_found",
						"Unable to find converter for " + type.getName());
			}
		}
		logger.debug("Found converter " + converter.getClass().getName());
		return converter;
	}
