...
import org.vraptor.view.OverridenViewManager;
...
public class ViewsPropertiesReader {
	private static final Logger logger = Logger
			.getLogger(ViewsPropertiesReader.class);
	...
	public ViewManager overrideViews(ViewManager currentManager,
		...
		OverridenViewManager newManager = new OverridenViewManager(
				currentManager);
		Properties properties = new Properties();
		properties.load(resource);
		logger.debug("loading view: " + properties.toString());
		for (Object key : properties.keySet()) {
			newManager.overrideView(key.toString(), properties.get(key)
					.toString());
		}
		return newManager;
	}
